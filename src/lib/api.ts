...
export async function getDreamLogs(): Promise<DreamLog[]> {
  ...
  return (data || []).map(log => ({
    ...
    entities: log.entities?.map((e: { entity: { name: string } }) => e.entity?.name).filter(Boolean) || [],
...
export async function addDreamLog(log: Omit<DreamLog, 'id' | 'createdAt'>): Promise<DreamLog | null> {
  ...
  if (log.world) {
    ...
    if (existingWorld) {
      ...
      const { data: newWorld } = await supabase
        .from('worlds')
        .insert({ name: log.world, type: 'transient', stability: 3 })
        .select('id')
        .single();
  ...
  for (const entityName of log.entities) {
    ...
    if (existingEntity) {
      ...
      const { data: newEntity } = await supabase
        .from('entities')
        .insert({ name: entityName, role: 'observer' })
        .select('id')
        .single();
...
export async function addSleepLog(log: Omit<SleepLog, 'id' | 'createdAt'>): Promise<SleepLog | null> {
  const { data, error } = await supabase
    .from('sleep_logs')
    .insert({
...
export async function getWorlds(): Promise<World[]> {
  ...
  return (data || []).map(world => ({
    ...
    dreamIds: world.dream_logs?.map((d: { dream_id: string }) => d.dream_id) || []
  }));
}

export async function addWorld(world: Omit<World, 'id' | 'dreamIds'>): Promise<World | null> {
  const { data, error } = await supabase
    .from('worlds')
    .insert({
      name: world.name,
...
export async function getEntities(): Promise<Entity[]> {
  ...
  return (data || []).map(entity => ({
    ...
    dreamIds: entity.dream_log_entities?.map((dle: { dream_log: { dream_id: string } }) => dle.dream_log?.dream_id).filter(Boolean) || []
  }));
}

export async function addEntity(entity: Omit<Entity, 'id' | 'dreamIds'>): Promise<Entity | null> {
  const { data, error } = await supabase
    .from('entities')
    .insert({
      name: entity.name,
...
export async function getModules(): Promise<SystemModule[]> {
  ...
  return (data || []).map(mod => ({
    ...
    dreamIds: mod.dream_log_modules?.map((dlm: { dream_log: { dream_id: string } }) => dlm.dream_log?.dream_id).filter(Boolean) || []
  }));
}
...
export async function getThreats(): Promise<ThreatEntry[]> {
  ...
  return (data || []).map(threat => ({
    ...
    dreamIds: threat.dream_log_threats?.map((dlt: { dream_log: { dream_id: string } }) => dlt.dream_log?.dream_id).filter(Boolean) || []
  }));
}
...
export async function parseSleepImage(imageBase64: string): Promise<{
  ...
  const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/parse-sleep`, {
    method: 'POST',
    headers: {
